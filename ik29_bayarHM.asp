<%response.cookies("ikmenu") = "ik29_bayarHM.asp"%>
<!-- '#INCLUDE FILE="ik.asp" -->
...
<td width="100%">
<form name=myform method="POST" action="ik29_bayarHM.asp">
<%
...
%>  <br/>
 <table width="50%" cellpadding="1" cellspacing="5" class="hd1">
    ...
    <td colspan=8>
      ...
      <a href="ik29_bayarHM.asp?page=1&bilangan=0&ms=pre&dtkh1=<%=tkh1%>&dtkh2=<%=tkh2%>&proses=Cari">
		<img name="firstrec" border="0" src="images/firstrec.jpg" width="20" height="20" alt="Halaman Mula"></a>  
      <% End If %>
      <% If iPageCurrent <> 1 Then%>
      <a href="ik29_bayarHM.asp?page=<%= iPageCurrent - 1 %>&bilangan=<%=bil-count-iPageSize%>&ms=pre&dtkh1=<%=tkh1%>&dtkh2=<%=tkh2%>&proses=Cari">
      ...
      <a href="ik29_bayarHM.asp?page=<%= iPageCurrent + 1 %>&bilangan=<%=bil%>&ms=next&dtkh1=<%=tkh1%>&dtkh2=<%=tkh2%>&proses=Cari">
      	<img name="next" border="0" src="images/next.jpg" width="20" height="20" alt="Rekod Seterusnya"></a> 
      <% End If 
	  If iPageCurrent < iPageCount Then
	  bil = (iPageCount - 1) * iPageSize %>
      <a href="ik29_bayarHM.asp?page=<%=iPageCount %>&bilangan=<%=bil%>&ms=next&dtkh1=<%=tkh1%>&dtkh2=<%=tkh2%>&proses=Cari">
...
<%		
		...
		'******************************************************************
		'mimi papar nama jabatan
				
		jabatan = Left(noakaun,5) 
		
		if jabatan = "76410" then 'jabatan perlesenan
		namaJabatan = "Jabatan Perlesenan"
		end if 
		
		if jabatan = "76101" then  'bangunan 
		namaJabatan = "Jabatan Bangunan"
		end if
		
		if jabatan = "76412" or jabatan = "76415"  or jabatan = "76416" then  'JAB PERKHIDMATAN PERBANDARAN 
		namaJabatan =  "Jabatan Perkhidmatan Perbandaran"
		end if 
		
		if jabatan = "76413" or jabatan = "76420" then  'JAB PERKHIDMATAN KESIHATAN
		namaJabatan = "Jabatan Perkhidmatan Kesihatan" 
		end if 
...
<form method="post" action="ik29r_bayarHM.asp"> 
 <table width="50%" cellpadding="1" cellspacing="5" class="hd1">
    <tr>
