<%response.cookies("ikmenu") = "ik216.asp"%>
<!-- '#INCLUDE FILE="ik.asp" -->
<%Response.Buffer = True%>
<html>
<head>
<title>i-Kompaun : Pengeluar Notis Mengikut Jabatan</title>
...
<form name="myform" method="POST" action="ik216.asp" >
<%
	'Set objConn = Server.CreateObject("ADODB.Connection")
'   objConn.Open "dsn=12c;uid=majlis;pwd=majlis;"
   
	response.cookies("amenu") = "ik216.asp"
...
elseif lokasi = "107" then
	...
	if proses = "Cari" then
		...
		'response.Write(jabatan)
		tkh1 = Request.form("tkhdari")	
		tkh2 = Request.form("tkhhingga")
	end if

	fparlimen1 = Request.QueryString("fparlimen")
	
	'response.write jabatan1
	
	if fparlimen1 <> "" then
		fparlimen = Request.QueryString("fparlimen1")
		response.Write(fparlimen)
 ...
 <table width="50%" cellpadding="1" cellspacing="5" class="hd">
  <tr> 
    ...
    <td>
       ...
       <select size="1" name="fparlimen">
          <%if fparlimen <> "" then
           zv = " select kod,initcap(keterangan) as ket from hasil.parlimen where kod = '"& fparlimen &"'  order by kod "
           Set rszv = objConn.Execute(zv) 
		   
		   if not rszv.bof and not rszv.eof then
		   %>
          <option value="<%=fparlimen%>"> <%=fparlimen%> - <%=rszv("ket")%> </option>
          <%   
	  end if
	  zv = " select kod,initcap(keterangan) ket from hasil.parlimen "
           zv = zv & "where kod <> '"& fparlimen &"' order by kod" 
           Set rszv = objConn.Execute(zv)
		  	
		else %>
          <option>Pilih Parlimen</option>
          <%   zv = " select kod,initcap(keterangan) ket from hasil.parlimen where kod not in ('109') order by kod " 
           Set rszv = objConn.Execute(zv)
		   kod = rszv("kod")
        end if
     ...
     loop 	%>
        ...
        </select><input type="hidden" value="<%=fparlimen%>" name="parlimen" >
</td>
</tr>
<!--<script>
	document.myform.jab.focus();
</script>-->
<tr >  
...
<script>
	document.myform.fparlimen.focus();
</script>
...
<%	Dim iPageSize,iPageCount,iPageCurrent,iRecordsShown
	...
	if proses = "Cari" or fparlimen <> "" or tkh1 <> "" or tkh2 <> "" then
	...
	if proses = "Cari" and fparlimen = "" then
		response.write "<script language=""javascript"">"
...
'response.write tkh2
		...
		'objRs2.Open d, objConn, adOpenStatic
 ...
 <table width="50%" cellpadding="1" cellspacing="5" class="hd1">
  <tr align="right">
    ...
    <td width="75%" >
      ...
      <a href="ik216.asp?page=1&bilangan=0&ms=pre&dtkh1=<%=tkh1%>&dtkh2=<%=tkh2%>&jabatan1=<%=fjabatan%>&proses=Cari">
		<img name="firstrec" border="0" src="images/firstrec.jpg" width="20" height="20" alt="Halaman Mula"></a>  
      <% End If %>
      <% If iPageCurrent <> 1 Then%>
      <a href="ik216.asp?page=<%= iPageCurrent - 1 %>&bilangan=<%=bil-count-iPageSize%>&ms=pre&dtkh1=<%=tkh1%>&dtkh2=<%=tkh2%>&jabatan1=<%=fjabatan%>&proses=Cari">
      ...
      <a href="ik216.asp?page=<%= iPageCurrent + 1 %>&bilangan=<%=bil%>&ms=next&dtkh1=<%=tkh1%>&dtkh2=<%=tkh2%>&jabatan1=<%=fjabatan%>&proses=Cari">
      	<img name="next" border="0" src="images/next.jpg" width="20" height="20" alt="Rekod Seterusnya"></a> 
      <% End If 
	  If iPageCurrent < iPageCount Then
	  bil = (iPageCount - 1) * iPageSize %>
      <a href="ik216.asp?page=<%=iPageCount %>&bilangan=<%=bil%>&ms=next&dtkh1=<%=tkh1%>&dtkh2=<%=tkh2%>&jabatan1=<%=fjabatan%>&proses=Cari">
...
<td width="5%" class="hd1">Tarikh</td>
<td width="5%" class="hd1" align="right">Amaun Bayar</td>
<td width="5%" class="hd1">No Resit</td>
<td width="10%" class="hd1">Jabatan</td>
<td width="10%" class="hd1">Dun</td>
  </tr>
<%		bil = 0
		ctrz = 0
		ab = 0
...
%>
 ...
 <td><%=objRs2("no_rujukan2")%></td>
  ...
  <td><%=objJab("keterangan")%></td>
  <td align="left"><%=ketDun%></td>
  </tr>
<%	iRecordsShown = iRecordsShown + 1
	count = count + 1
...
<form method="post" action="ik216r.asp"> 
 <table width="50%" cellpadding="1" cellspacing="5" class="hd1">
    <tr>
...
<input type="hidden" name="fparlimen11" value="<%=fparlimen%>">
